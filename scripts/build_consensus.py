...
def main():
        ...
        for line in mapfile:
            ...
            if line.startswith("@"):
                ...
                if line.startswith("@SQ"):
                    ...
                    sequences[refname] = [
                        {"-": 0, "A": 0, "C": 0, "G": 0, "N": 0, "T": 0} for nuc in range(reflength)]
    ...
    print("References: " + refname + ", Length: " + str(reflength))
    mapfile.close()
        ...
        while 1:
            ...
            for line in mapfile_chunk:
                ...
                if line[0] != "@" and line.split("\t")[5] != "*":
                    ...
                    # Reference name, skip description if mapper kept it
                    ...
                    seqout, insert = parsecigar(
                        sam_record[5], sam_record[9], pos_ref)
    ...
    print(("A total of "+str(reads_total)+" reads were processed, out of which, " +
          str(reads_mapped)+" reads were mapped.\n"))
    mapfile.close()
    ...
    for refname in sequences:
        ...
        if insertions[refname] != []:
            ...
            for pos in sorted(ins_tmp1):
                ...
                ins_tmp2[pos] = [{"-": 0, "A": 0, "C": 0, "G": 0,
                                  "N": 0, "T": 0} for i in range(motif_max_len)]
            ...
            for pos in sorted(ins_tmp2):
                for col in range(len(ins_tmp2[pos])):
                    ...
                    ins_tmp2[pos][col]["-"] = coverages[refname][pos] - \
                        sum(ins_tmp2[pos][col].values())
    ...
    for reference in fastas:
        ...
        if nchar == 0:
            ...
            outfile.write("\n".join([i[0]+"\n"+"\n".join([i[1][s:s+nchar]
                          for s in range(0, len(i[1]), nchar)]) for i in fastas[reference]])+"\n")
        outfile.close()
        if len(thresholds) == 1:
